...
if (preg_match($pattern, $mb_id) || preg_match($pattern, $mb_password)) {
    ...
    ('고객사', 'login', '{$mb['mb_no']}','{$mb['mb_name']}','검증되지않은 로그인시도1 (로그인실패)','{$ip}');
    ";
    isSqlError(sql_query($record_hist_sql), $record_hist_sql);
    exit('검증되지않은 로그인시도1 (로그인실패)');
...
if (preg_match($script_pattern, $mb_id) || preg_match($script_pattern, $mb_password)) {
    ...
    $record_hist_sql = "
    insert into gnp_record_hist (hist_join_gubun, hist_function, hist_mb_no, hist_mb_name, hist_detail, client_ip) values
    ('고객사', 'login', '{$mb['mb_no']}','{$mb['mb_name']}','검증되지않은 로그인시도2 (로그인실패)','{$ip}');
    ";
    isSqlError(sql_query($record_hist_sql), $record_hist_sql);
    exit('검증되지않은 로그인시도2 (로그인실패)');
}
...
if (!$mb || empty($mb['mb_no'])) {
    ...
    $record_hist_sql = "
    insert into gnp_record_hist (hist_join_gubun, hist_function, hist_mb_no, hist_mb_name, hist_detail, client_ip) values
    ('고객사', 'login', NULL, NULL, '회원 정보를 찾을 수 없습니다.', '{$ip}');
    ";
    isSqlError(sql_query($record_hist_sql), $record_hist_sql);
    exit('회원 정보를 찾을 수 없습니다.');
}
...
if (!$is_social_password_check && (! (isset($mb['mb_id']) && $mb['mb_id']) || !login_password_check($mb, $mb_password, $mb['mb_password'])) ) {
    ...
    $record_hist_sql = "
    insert into gnp_record_hist (hist_join_gubun, hist_function, hist_mb_no, hist_mb_name, hist_detail, client_ip) values
    ('고객사', 'login', '{$mb['mb_no']}','{$mb['mb_name']}','로그인 아이디|비밀번호 다름','{$ip}');
    ";
    isSqlError(sql_query($record_hist_sql), $record_hist_sql);
...
if ($mb['mb_intercept_date'] && $mb['mb_intercept_date'] <= date("Ymd", G5_SERVER_TIME)) {
    ...
    $record_hist_sql = "
    insert into gnp_record_hist (hist_join_gubun, hist_function, hist_mb_no, hist_mb_name, hist_detail, client_ip) values
    ('고객사', 'login', '{$mb['mb_no']}','{$mb['mb_name']}','접근금지 아이디 접속','{$ip}');
    ";
    isSqlError(sql_query($record_hist_sql), $record_hist_sql);
...
if ($mb['mb_leave_date'] && $mb['mb_leave_date'] <= date("Ymd", G5_SERVER_TIME)) {
    ...
    $record_hist_sql = "
    insert into gnp_record_hist (hist_join_gubun, hist_function, hist_mb_no, hist_mb_name, hist_detail, client_ip) values
    ('고객사', 'login', '{$mb['mb_no']}','{$mb['mb_name']}','탈퇴 아이디 접속','{$ip}');
    ";
    isSqlError(sql_query($record_hist_sql), $record_hist_sql);
...
if ($mb['is_login'] == "N") {
    ...
    $record_hist_sql = "
    insert into gnp_record_hist (hist_join_gubun, hist_function, hist_mb_no, hist_mb_name, hist_detail, client_ip) values
    ('고객사', 'login', '{$mb['mb_no']}','{$mb['mb_name']}','가입대기중인 계정','{$ip}');
    ";
    isSqlError(sql_query($record_hist_sql), $record_hist_sql);
...
$record_hist_sql = "
insert into gnp_record_hist (hist_join_gubun, hist_function, hist_mb_no, hist_mb_name, hist_detail, client_ip) values
('고객사', 'login', '{$mb['mb_no']}','{$mb['mb_name']}','로그인성공','{$ip}');
";
isSqlError(sql_query($record_hist_sql), $record_hist_sql);
