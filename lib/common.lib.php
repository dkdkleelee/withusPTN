...
function get_member($mb_id, $fields='*', $is_cache=false)
{
    ...
    $sql = " select $fields from {$g5['member_table']} where mb_id = TRIM('$mb_id') and mb_gubun != 'E'";
...
function view_dateformat($date)
{
    ...
    if($param == $today) {
        $str = date("H:i:s", strtotime($date)). ' <img src="'.G5_THEME_URL.'/img/new.gif" alt="새글">' ;
    } else {
        $str = date("d일 H시", strtotime($date));
    }
    return $str;
}

//날짜 하루전거면 MM일 HH시 당일이면 등록시
function view_dateformat2($date)
{
    $str = "";
    ...
    if($param == $today) {
        $str = date("m월d일 H:i:s", strtotime($date)). ' <img src="'.G5_THEME_URL.'/img/new.gif" alt="새글">' ;
    } else {
        $str = date("y년m월d일 H:i:s", strtotime($date));
    }
    return $str;
}
