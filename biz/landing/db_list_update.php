...
if($act_button == "엑셀다운") {
    ...
    if($stx_phone == "" && $stx_name == "" && $stx_db_status == "" && $stx_fromto == "") {
        ...
        if($stx_phone) {
            ...
            $sql_search .= "and phone = '$stx_phone' ";
        } 
        if($stx_name) {
            $hist_memo .= "이름:{$stx_name} || ";
            $sql_search .= "and userName = '$stx_name' ";
        } 
        if($stx_db_status) {
    ...
    // dynamicColumnCount에 따라 userData 컬럼 추가
